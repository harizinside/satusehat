 ...
 *
 * Source: [PUBLIC] ICD-9CM e-klaim.xlsx - ICD9 CM.csv (Kemkes e-klaim ICD9CM_2010), 4626 entries.
 * Static data only — no filesystem access at runtime.
 */
import { SYSTEM } from "./systems.js";
import type { Coding } from "../types.js";

/** All ICD9CM codes and displays, in source (alphabetical) order. */
const ENTRIES: readonly (readonly [string, string])[] = [
    ...
    ["03.39", "Other diagnostic procedures on spinal cord and spinal canal structure"],
    ...
    ["37.62", "Insertion of temporary non-implantable extracorporeal heart circulatory assist device"],
    ...
    ["38.52", "Ligation and stripping of varicose veins, other vessels of head and neck "],
...
/** Look up one ICD9CM code; undefined if unknown. */
export function icd9cmCoding(code: string): Coding | undefined {
  const display = LOOKUP.get(code);
  return display === undefined ? undefined : { system: SYSTEM.ICD9CM, code, display };
}

/** Case-insensitive substring search over displays (cheap linear scan). */
export function searchIcd9Cm(query: string, limit = 20): Coding[] {
  const q = query.toLowerCase();
  const hits: Coding[] = [];
  if (q !== "") {
    for (const [code, display] of ENTRIES) {
      if (display.toLowerCase().includes(q)) {
        hits.push({ system: SYSTEM.ICD9CM, code, display });
...
export const ICD9CM_COUNT = ENTRIES.length;
